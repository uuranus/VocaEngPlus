...
class MainActivity : AppCompatActivity() {
    ...
    private fun initDrawer(){
        ...
        navView.setNavigationItemSelectedListener {
            ...
            if(id== R.id.nav_profile){
                ...
                if(!initialization.getFBuser().isAnonymous){
                    initialization.getFBauth().sendPasswordResetEmail(initialization.getFBuser().email.toString())
    ...
    private fun init() {
        ...
        val wordbtn=findViewById<ImageView>(R.id.wordbtn)
        val testbtn=findViewById<ImageView>(R.id.testbtn)
        val reviewbtn=findViewById<ImageView>(R.id.reviewbtn)
        val mywordbtn=findViewById<ImageView>(R.id.mywordbtn)
        val naverbtn=findViewById<ImageView>(R.id.naverbtn)
        val statisticsbtn=findViewById<ImageView>(R.id.statisticsbtn)
        val communitybtn=findViewById<ImageView>(R.id.communitybtn)
    ...
    private fun AlertDelete() {
        ...
        builder.setMessage("해당 계정을 삭제하시겠습니까?")
            .setPositiveButton("네"){ _, _ ->
                ...
                initialization.databaseref.child("UserDownload").child(initialization.uid).removeValue()
                initialization.databaseref.child("Community").child(initialization.uid).removeValue()
                ...
                initialization.firebaseUser.delete().addOnCompleteListener {
                    if(it.isSuccessful){
                        ...
                        val snackbar = Snackbar.make(drawerLayout, "로그인한 지 오래되었습니다.\n다시 로그인해주세요", Snackbar.LENGTH_INDEFINITE)
    ...
    private fun SendEmailToDev(){
        ...
        email.putExtra(Intent.EXTRA_TEXT,String.format("문의 시, 아래 내용을 함께 보내주시면 큰 도움이 됩니다.\n**********\n "+
                " 사용자 ID: %s\n 앱 버전 : %s\n 기기 : %s\n 안드로이드 OS 버전 : %d(%s)\n ",initialization.firebaseUser.uid,
            BuildConfig.VERSION_NAME,
            Build.DEVICE,Build.VERSION.SDK_INT,Build.VERSION.RELEASE
        ))
