...
class WordActivity : AppCompatActivity() {
    ...
    private fun init() {
        ...
        databaseref.child("UserData").child(firebaseUser.uid).child("downloadNames")
            .get().addOnSuccessListener {
                ...
                binding.wordSpinner.adapter =
                    ArrayAdapter(this, R.layout.simple_dropdown_item_1line, categories)
                binding.wordSpinner.setSelection(isSelected)
        ...
        binding.addVocaButton.setOnClickListener {
                ...
                .setPositiveButton("추가") { _, _ ->
                    ...
                    if (word.length == 0 || meaning.length == 0) {
                        ...
                        if (!validation.isValidateWord(word)) {
                            Toast.makeText(this, "영단어 입력이 올바르지 않아 추가에 실패하였습니다.", Toast.LENGTH_SHORT)
                                .show()
                        } else if (!validation.isValidateMeaning(meaning)) {
                            Toast.makeText(this, "뜻 입력이 올바르지 않아 추가에 실패하였습니다.", Toast.LENGTH_SHORT)
                                .show()
                        } else {
