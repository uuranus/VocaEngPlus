                        ...
                        .setPositiveButton("확인"){
                            _,_ ->
                                ...
                                if(!validation.isValidateEmail(email)){
                                    Toast.makeText(this@LoginActivity,"이메일이 올바르지 않습니다.",Toast.LENGTH_SHORT).show()
                                    return@setPositiveButton
                                }
                                Toast.makeText(this@LoginActivity,"비밀번호 재설정 이메일을 전송하였습니다.",Toast.LENGTH_SHORT).show()
                                initialization.getFBauth().sendPasswordResetEmail(email)
                                        .addOnSuccessListener { it2->
                                                Toast.makeText(this@LoginActivity, "비밀번호 재설정 이메일을 보냈습니다.", Toast.LENGTH_SHORT).show()
                                        }
                                        .addOnCanceledListener {
                                            Toast.makeText(this@LoginActivity, "비밀번호 재설정 이메일을 보내는데 실패했습니다..", Toast.LENGTH_SHORT).show()
                                        }
                        ...
                        .setPositiveButton("확인"){
                            ...
                            firebaseauth.signInAnonymously().addOnCompleteListener { task ->
                                if (task.isSuccessful) {
                                    ...
                                    Toast.makeText(this@LoginActivity, "게스트 가입 실패", Toast.LENGTH_SHORT).show()
                                }
                            }
