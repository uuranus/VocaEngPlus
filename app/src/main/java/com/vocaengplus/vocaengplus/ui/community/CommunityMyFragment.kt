...
class CommunityMyFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        Initialization.getDBref().child("UserData").child(Initialization.uid).child("downloadData")
            ...
            for (cld in it.children) {
                if (cld.child("categorywritertoken").value == Initialization.uid && cld.hasChild("words")) {
                    ...
                    for (c in cld.child("words").children) {
                        words.add(
                            Voca(
                                c.child("category").value.toString(),
                                c.child("word").value.toString(),
                                c.child("meaning").value.toString(),
                                0
                            )
                        )
                    }
                    ...
                    val category = CommunityForUpload(
                        cld.child("categoryname").value.toString(),
                        Initialization.getFBuser().photoUrl.toString(),
                        cld.child("categorywriter").value.toString(),
                        cld.child("categorywritertoken").value.toString(),
                        Initialization.getdate(),
                        cld.child("description").value.toString(),
                        0,
                        isliked,
                        0,
                        isDownloaded,
            ...
            Initialization.getDBref().child("Community").child(Initialization.uid).get()
    ...
    private fun initRecyclerView(data: ArrayList<CommunityForUpload>, uploads: ArrayList<String>) {
        ...
        adapter.itemClickListener = object : CommunityMyAdapter.OnItemClickListener {
            override fun OnItemClick(
                holder: CommunityMyAdapter.ViewHolder,
                view: View,
                position: Int
            ) {
                if (Initialization.getFBuser().isAnonymous) {
                ...
                if (view.isSelected) {
                    ...
                    builder.setMessage("해당 단어장을 올리시겠습니까?")
                        .setPositiveButton("네") { _, _ ->
                            Initialization.getDBref().child("Community").child("categories").get()
                                .addOnSuccessListener {
                                    ...
                                    for (i in 0..data[position].words.size - 1) {
                                        words.put(
                                            data[position].words[i].word,
                                            Voca(
                                                data[position].words[i].category,
                                                data[position].words[i].word,
                                                data[position].words[i].meaning,
                                                0
                                            )
                                        )
                                    }
                                    Initialization.getDBref().child("Community").child("categories")
                                        .child(data[position].categoryname)
                                        .setValue(
                                            CommunityForLikeDownload(
                                                data[position].categoryname,
                                                Initialization.getFBuser().photoUrl.toString(),
                                                data[position].categorywriter,
                                                Initialization.uid,
                                                Initialization.getdate(),
                                                ...
                                                data[position].likes,
                                                0,
                                                data[position].downloads,
                                                words
                                            )
                                        )
                                    Initialization.getDBref()
                                        .child("Community").child(Initialization.uid)
                                        .child("totalUpload").get().addOnSuccessListener {
                                            var tu = it.value.toString().toInt()
                                            Initialization.getDBref()
                                                .child("Community").child(Initialization.uid)
                                                .child("totalUpload").setValue(++tu)
                                            Initialization.getDBref()
                                                .child("Community").child(Initialization.uid)
                                                .child("uploads").child(data[position].categoryname)
                                                .setValue(true)
                                            initTable()
                                        }
