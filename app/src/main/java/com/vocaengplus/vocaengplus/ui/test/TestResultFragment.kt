...
class TestResultFragment(
    ...
    private val corcount: Int,
    private val resultdata: ArrayList<Voca>
) : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val tc = view.findViewById<TextView>(R.id.totalCountTextView)
        val cc = view.findViewById<TextView>(R.id.correctCountTextView)
        val wc = view.findViewById<TextView>(R.id.wrongCountTextView)
        ...
        val btn = view.findViewById<Button>(R.id.endTestButton)
        btn.setOnClickListener {
            ...
            databaseref.child("UserLog").child(uid).get().addOnSuccessListener { snapshot ->
                if (!snapshot.hasChild(date.substring(0, 7))) {
                    ...
                    var testcount = snapshot.child(date.substring(0, 7)).child("Test")
                        .child("testcount").value.toString().toInt()
                    var tc = snapshot.child(date.substring(0, 7)).child("Test")
                        .child("totalcount").value.toString().toInt()
                    var cc = snapshot.child(date.substring(0, 7)).child("Test")
                        .child("corcount").value.toString().toInt()
                    var wc = snapshot.child(date.substring(0, 7)).child("Test")
                        .child("wrongcount").value.toString().toInt()

                    //테스트 정보값 갱신
                    databaseref.child("UserLog").child(uid).child(month).child("Test")
                        .child("testcount").setValue(++testcount)
                    databaseref.child("UserLog").child(uid).child(month).child("Test")
                        .child("totalcount").setValue(tc + wrongcount + corcount)
                    databaseref.child("UserLog").child(uid).child(month).child("Test")
                        .child("corcount").setValue(cc + corcount)
                    databaseref.child("UserLog").child(uid).child(month).child("Test")
                        .child("wrongcount").setValue(wc + wrongcount)
                    ...
                    if (i.checked == 0) { //오답이라면
                        if (testcategory != "오답노트") {
                            databaseref.child("UserData").child(uid.toString())
                                .child("downloadData").child("오답노트").child("words")
                                .child(i.category + " " + i.word)
                                .setValue(Voca(i.category, i.word, i.meaning, 0))
                        }
                        //오답노트 아니면 그냥 pass
...
//                (activity as TestActivity).replaceFragment(TestMainFragment(), "testmain")
    ...
    private fun initData() {
        ...
        databaseref.child("UserLog").child(uid).child(date.substring(0, 7)).child("Test")
        ...
        databaseref.child("UserLog").child(uid).child(date.substring(0, 7)).child("AddDelete")
            .child("addCategory").setValue(0)
        databaseref.child("UserLog").child(uid).child(date.substring(0, 7)).child("AddDelete")
            .child("deleteCategory").setValue(0)
        databaseref.child("UserLog").child(uid).child(date.substring(0, 7)).child("AddDelete")
            .child("addWord").setValue(0)
        databaseref.child("UserLog").child(uid).child(date.substring(0, 7)).child("AddDelete")
            .child("deleteWord").setValue(0)
        initlog()
    }
    ...
    private fun initlog() {
        ...
        val date = initialization.getdate()
        val month = date.substring(0, 7)
        databaseref.child("UserLog").child(uid).child(month).child("Test").get()
            .addOnSuccessListener {
                if (it.hasChild("log")) {
                    if (it.child("log").hasChild(date)) {
                        var testcount =
                            it.child("log").child(date).child("testcount").value.toString().toInt()
                        var tc =
                            it.child("log").child(date).child("totalcount").value.toString().toInt()
                        var cc =
                            it.child("log").child(date).child("corcount").value.toString().toInt()
                        var wc =
                            it.child("log").child(date).child("wrongcount").value.toString().toInt()

                        //테스트 정보값 갱신
                        databaseref.child("UserLog").child(uid).child(month).child("Test")
                            .child("log").child(date).child("testcount").setValue(++testcount)
                        databaseref.child("UserLog").child(uid).child(month).child("Test")
                            .child("log").child(date).child("totalcount")
                            .setValue(tc + wrongcount + corcount)
                        databaseref.child("UserLog").child(uid).child(month).child("Test")
                ...
                databaseref.child("UserLog").child(uid).child(month).child("Test").child("log")
                    .child(date)
                    .child("testcount").setValue(1)
                databaseref.child("UserLog").child(uid).child(month).child("Test").child("log")
                    .child(date)
