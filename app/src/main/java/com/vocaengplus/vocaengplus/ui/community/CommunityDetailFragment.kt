...
class CommunityDetailFragment(val data: CommunityCategory) : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding!!.apply {
                ...
                deleteButton.setOnClickListener {
                    ...
                    builder.setMessage("해당 단어장을 삭제하시겠습니까?")
                        .setPositiveButton("네") { _, _ ->
                            Initialization.getDBref().child("Community").child("categories")
                                .child(data.categoryname).removeValue()
                            Initialization.getDBref().child("Community").child(Initialization.uid)
                                .child("uploads").child(data.categoryname).removeValue()
                            (activity as CommunityActivity).replaceFragment(
                                CommunityMainFramgent(),
                                "communitymain"
                            ) //메인화면으로 돌아가기
                        }
            ...
            titleTextView.text = data.categoryname.toString()
            writerTextView.text = "${data.categorywriter}"
            uploadDateTextView.text = "업로드  ${data.uploadDate}"
            descriptionTextView.text = data.description
            likeTextView.text = "${data.like}"
            downloadTextView.text = "${data.download}"
            ...
            if (data.isDownloaded) {
                downloadImageView.isSelected = true
                isdonwload = true
            } else {
                downloadImageView.isSelected = false
                isdonwload = false
            }
            ...
            binding!!.likeImageView.setOnClickListener {
                if (Initialization.getFBuser().isAnonymous) {
                    ...
                    updateHeart(
                        Initialization.getDBref().child("Community").child("categories")
                            .child(data.categoryname.toString())
                    )
                }
            }
                ...
                if (Initialization.getFBuser().isAnonymous) {
                    ...
                    if (isdonwload) {
                        ...
                        builder.setMessage("해당 단어장을 다운받으시겠습니까?")
                            .setPositiveButton("네") { _, _ ->
                                binding!!.downloadImageView.setImageResource(R.drawable.ic_check)
                                updateDownload(
                                    Initialization.getDBref().child("Community").child("categories")
                                        .child(data.categoryname.toString())
                                )
        ...
        categoryRef.runTransaction(object : Transaction.Handler {
            override fun doTransaction(currentData: MutableData): Transaction.Result {
                ...
                if (p.likes!!.containsKey(Initialization.uid)) { //좋아요를 눌렀다면
                    ...
                    Initialization.getDBref().child("Community").child(Initialization.uid)
                        .child("totalLike").get().addOnSuccessListener {
                            var likecount = it.value.toString().toInt()
                            if (likecount == 1) {
                                Initialization.getDBref().child("Community")
                                    .child(Initialization.uid)
                                    .child("totalLike").setValue(0)
                            } else {
                                Initialization.getDBref().child("Community")
                                    .child(Initialization.uid)
                                    .child("totalLike").setValue(--likecount)
                            }
                        }
                    if (data.categorywritertoken != "Default") {
                        updateWriter(
                            Initialization.getDBref().child("Community")
                                .child(data.categorywritertoken), true, false
                        )
                    }
                } else {
                    p.like = p.like + 1
                    ...
                    if (data.categorywritertoken != "Default") {
                        updateWriter(
                            Initialization.getDBref().child("Community")
                                .child(data.categorywritertoken), true, true
                        )
                    }
                }
            ...
            override fun doTransaction(currentData: MutableData): Transaction.Result {
                ...
                if (!(p.downloads!!.containsKey(Initialization.uid))) { //다운로드를 눌렀다면
                    ...
                    if (data.categorywritertoken != "Default") {
                        updateWriter(
                            Initialization.getDBref().child("Community")
                                .child(data.categorywritertoken), false, true
                        )
                    }
                }
    ...
    private fun initLikeDownload() {
        Initialization.getDBref().child("Community").child("categories").child(data.categoryname)
            .get().addOnSuccessListener {

                binding!!.likeTextView.text = "${it.child("like").value}"
                binding!!.downloadTextView.text = "${it.child("download").value}"
            }
    }

    private fun downloadCategory() {
        Initialization.getDBref().child("UserData").child(Initialization.uid).child("downloadData")
            .child(data.categoryname).setValue(
                Category(
                    data.categoryname, data.categorywriter, data.categorywritertoken,
                    Initialization.getdate(), data.description, ArrayList<Voca>()
                )
            )
        Initialization.getDBref().child("UserData").child(Initialization.uid).child("downloadNames")
            .child(data.categoryname).setValue(data.categoryname)
        for (i in 0..data.words.size - 1) {
            Initialization.getDBref().child("UserData").child(Initialization.uid)
                .child("downloadData").child(data.categoryname).child("words")
                .child(data.words[i].word).setValue(
        ...
        Initialization.getDBref()
            .child("UserLog").child(Initialization.uid).get().addOnSuccessListener { snapshot ->
                if (!snapshot.hasChild(Initialization.getdate().substring(0, 7))) {
                    ...
                    var ac =
                        snapshot.child(Initialization.getdate().substring(0, 7)).child("AddDelete")
                            .child("addCategory").value.toString().toInt()
                    Initialization.getDBref().child("UserLog").child(Initialization.uid).child(
                        Initialization.getdate().substring(0, 7)
                    ).child("AddDelete").child("addCategory").setValue(++ac)
                }
            }
    }
}