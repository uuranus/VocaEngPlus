...
class AddCategory : AppCompatActivity() {
    ...
    fun init(){
        ...
        binding.addokbtn.setOnClickListener {
            ...
            databaseref.child("UserData").child(uid.toString())
                    .child("downloadData").child(categoryname).setValue(Category(categoryname,initialization.getFBuser().displayName.toString(),initialization.uid,date,description,ArrayList<Voca>()))

            for(i in 0 until wordlist.size){
                databaseref.child("UserData").child(uid.toString())
                        .child("downloadData").child(categoryname).child("words").child(wordlist.get(i).word).setValue(Voca(categoryname,wordlist.get(i).word,wordlist.get(i).meaning,0))
            }
