...
class CommunityDetailFragment(val data:CommunityCategory) : Fragment() {
    ...
    private fun downloadCategory(){
        initialization.getDBref().child("UserData").child(initialization.uid).child("downloadData").child(data.categoryname).setValue(Category(data.categoryname,data.categorywriter,data.categorywritertoken,initialization.getdate(),data.description,ArrayList<Voca>()))
        ...
        for(i in 0..data.words.size-1){
            initialization.getDBref().child("UserData").child(initialization.uid).child("downloadData").child(data.categoryname).child("words").child(data.words[i].word).setValue(Voca(data.words[i].category,data.words[i].word,data.words[i].meaning,0))
        }

        //log값에 download한 것 추가
        initialization.getDBref().child("UserLog").child(initialization.uid).get().addOnSuccessListener { snapshot ->
            if (!snapshot.hasChild(initialization.getdate().substring(0, 7))) {
                ...
                initialization.getDBref().child("UserLog").child(initialization.uid).child(initialization.getdate().substring(0, 7)).child("AddDelete").child("addCategory").setValue(++ac)
            }
        }
    }
}