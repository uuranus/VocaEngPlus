        ...
        initialization.getDBref().child("UserData").child(initialization.uid).child("downloadData").get().addOnSuccessListener {
            for(cld in it.children){
                if(cld.child("categorywritertoken").value==initialization.uid&&cld.hasChild("words")){
                    ...
                    val category=CommunityForUpload(cld.child("categoryname").value.toString(),initialization.getFBuser().photoUrl.toString(),cld.child("categorywriter").value.toString(),
                            cld.child("categorywritertoken").value.toString(),initialization.getdate(),cld.child("description").value.toString()
                            ...
                            .setPositiveButton("네"){ _, _ ->
                                initialization.getDBref().child("Community").child("categories").get().addOnSuccessListener {
                                    ...
                                    for(i in 0..data[position].words.size-1){
                                        words.put(data[position].words[i].word,Voca(data[position].words[i].category,data[position].words[i].word,data[position].words[i].meaning,0))
                                    }
                                    initialization.getDBref().child("Community").child("categories").child(data[position].categoryname)
                                            .setValue(CommunityForLikeDownload(data[position].categoryname,initialization.getFBuser().photoUrl.toString(),data[position].categorywriter,initialization.uid,initialization.getdate(),data[position].description,0,data[position].likes,0,data[position].downloads,words))
                                    initialization.getDBref().child("Community").child(initialization.uid).child("totalUpload").get().addOnSuccessListener {
                                        var tu=it.value.toString().toInt()
                                        initialization.getDBref().child("Community").child(initialization.uid).child("totalUpload").setValue(++tu)
                                        initialization.getDBref().child("Community").child(initialization.uid).child("uploads").child(data[position].categoryname).setValue(true)
                                        initTable()
                                    }
