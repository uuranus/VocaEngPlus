        ...
        binding.apply{
            ...
            savebtn.setOnClickListener {
                ...
                if(newnickname.length==0){
                    Toast.makeText(this@Profile, "닉네임을 입력해주세요.", Toast.LENGTH_SHORT).show()
                    return@setOnClickListener
                }
                else if(!validation.isValidateNickname(newnickname)){
                    Toast.makeText(this@Profile, "닉네임은 2~8글자 사이여야 합니다.(특수문자 불가).", Toast.LENGTH_SHORT).show()
                    return@setOnClickListener
                }
                else {
                            ...
                            .addOnCompleteListener {
                                if(it.isSuccessful){
                                    ...
                                    databaseref.child("Community").child(firebaseUser.uid).get().addOnSuccessListener {
                                        ...
                                        Toast.makeText(this@Profile, "계정 수정 완료", Toast.LENGTH_SHORT).show()
                                        setResult(RESULT_OK)
                                        finish()
                                    }
                                }
